    ...
    <div id="items" class="fbox-v">
      <router-link class="router-link" ref="home" to="/">
        <svg
          ...
          viewBox="0 0 640 512"
          class="svg-inline--fa fa-house-night fa-w-20"
        >
          <path
            fill="currentColor"
      ...
      <router-link class="router-link" ref="projects" to="/projects"
...
export default class App extends Vue {
  mounted(): void {
    router.afterEach((to) => {
      ...
      let id = (to.name as string).toLowerCase();
      if (id.includes("blog")) id = "blog";
      let page = this.$refs[id] as Vue;

      for (const ref in this.$refs) {
        (this.$refs[ref] as Vue).$el.classList.remove("active");
      }
