<template>
    <div id="nav" class="fbox-v" :class="(currentRoute) + ' ' + (menuOpen ? 'open' : '')"
        ...
        <div id="items" class="fbox-v">
            <router-link class="router-link" ref="home" to="/">
                <svg focusable="false" data-prefix="fal" data-icon="house-night" role="img"
                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="svg-inline--fa fa-house-night fa-w-20">
            ...
            <router-link class="router-link" ref="blog" to="/blog">Blog</router-link>
            <router-link class="router-link" ref="projects" to="/projects">Projects</router-link>
        </div>
...
export default class App extends Vue {
    ...
    updateBookmark(to: RouteLocationNormalized): void {
        this.$nextTick(() => {
            ...
            document.title = to.meta.title ? String(to.meta.title) : 'Home';
        })

        this.currentRoute = ((to.meta?.navBookmark ?? to.name) as string).toLowerCase()

        if (this.currentRoute.includes('blog')) this.currentRoute = 'blog'
